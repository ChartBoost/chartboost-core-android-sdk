...
class UtilsTest {
    ...
    @Test
    fun `exponential backoff succeeds after one attempt if action succeeds`() =
        runTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `exponential backoff fails after max attempts`() =
        runTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `exponential backoff takes at least the minimum expected duration`() =
        runTest {
            var attempts = 0
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `exponential backoff can handle exceptions thrown by action`() =
        runTest {
            ...
            assertTrue(result.exceptionOrNull() is ChartboostCoreException)
            assertTrue(
                (result.exceptionOrNull() as ChartboostCoreException).error is ChartboostCoreError.CoreError.ExponentialBackoffRetriesExhausted,
            )
        }

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `safeExecute runs given block`() =
        runTest {
